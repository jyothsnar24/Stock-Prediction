...
def predict_stock_price(symbol):
  ...
  prediction_data = pd.DataFrame({'Date': pd.date_range(start=now + pd.DateOffset(days=1), periods=7), 'SARIMAX Predictions': predictions})
...
def main():
  ...
  # Provide a list of sample stock names
  sample_stock_names = ["AAPL"]
  ...
  if stock_data is not None:
    ...
    # Display a button to predict the future stock price
    if st.button("Predict"):
      # Add a loading state while predicting
      with st.spinner("Predicting..."):
      ...
      # Check if the prediction data is available
      if prediction_data is not None:
